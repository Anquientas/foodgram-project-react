...
Пользователям сайта также доступен сервис «Список покупок». Он позволяет создавать список продуктов, которые нужно купить для приготовления добавленных в спсиок блюд.
...
- создать в корневой директории проекта и заполнить файл ```.env``` параметрами, указанными в файле ```.env.example``` (см. подраздел "**Параметры .env**"), расположенный в корневой директории проекта;
